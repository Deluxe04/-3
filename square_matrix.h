...
public:
    ...
    SquareMatrix<T> Map(T (*f)(const T&)) const 
    {
        SquareMatrix<T> res(n);
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j)
                res.Set(i, j, f(uncheckedGet(i, j)));
        return res;
    }
    ...
    T Reduce(T (*f)(const T&, const T&), T init) const 
    {
        ...
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j)
                acc = f(acc, uncheckedGet(i, j));
        return acc;
    }

    DynamicArray<T> Where(bool (*pred)(const T&)) const 
    {
        int count = 0;
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j)
                if (pred(uncheckedGet(i, j))) ++count;

        DynamicArray<T> res(count);
        int idx = 0;
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j)
                if (pred(uncheckedGet(i, j))) res.Set(idx++, uncheckedGet(i, j));
        return res;
    }
